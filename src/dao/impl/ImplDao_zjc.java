...
public class ImplDao_zjc implements IDao_zjc {
	...
	public List<DeptDict> getDepts() {
		...
		// 显示有预约科室的时间
		// String hql =
		// "from DeptDict where dept_code in (select clinicDept from OutpDoctorRegist where to_date(sysdate) <= COUNSEL_DATE) ";
		String hql = "from DeptDict where dept_code in (select clinicDept from OutpDoctorRegist where to_date(sysdate) <= COUNSEL_DATE)";

		// 显示全部科室（暂用）
		// String hql =
		// "from DeptDict where dept_code in (select clinicDept from OutpDoctorRegist ) ";
	...
	// 查排班
	@SuppressWarnings("unchecked")
	public List<OutpDoctorRegist> CheckOnDuty(Integer dayNum,
			String clinic_dept, String doctor_no) {
	...
	public OutpDoctorRegist getOutpDoctorOne(String doctorName, String date,
			String duration) {
		String hql = "from OutpDoctorRegist where doctor='" + doctorName
				+ "' and CLINIC_DURATION ='" + duration
				+ "' and to_date(COUNSEL_DATE) = to_date('" + date
				+ "','yyyy-MM-dd')";
		...
		Query query = session
				.createSQLQuery(
						"SELECT T_A.TIMES, to_char(T_A.TIMES, 'hh24:mi') as regist_time, to_char(T_A.TIMES, 'yyyy-mm-dd hh24:mi') as real_regist_time, T_A.SQNO "
								...
								+ "order by T_A.times")
				.setParameter(0, counsel_date).setParameter(1, doctor_no)
				.setParameter(2, clinic_duration).setParameter(3, counsel_date)
				.setParameter(4, doctor_no).setParameter(5, queue_name)
				.setParameter(6, clinic_duration).setParameter(7, doctor_no)
	...
	public void addOne(String user_id) {
		Session session = HibernateUtil.getSession();
		List<PatMasterIndex> pmis = null;
		PatMasterIndex pmi = null;

		Query query = session.createQuery(strByUserId);
